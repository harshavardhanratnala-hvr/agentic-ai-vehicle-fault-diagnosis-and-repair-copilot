...
- **Who exactly is it for?** Fleet maintenance manager, primary — watching continuous telemetry, their question is "this vehicle's readings are trending toward a fault, do I bring it in proactively, or is it fine?" OEM/dealer diagnostic engineer, secondary — a fault has already fired by the time they're involved, so theirs is reactive: "what's the root cause, has it been seen before, what's the fix?"
- **Pre-production or post-production?** Post-production, in-service. This diagnoses vehicles already built and on the road — whether that's a live fault code that just fired, or telemetry already trending toward one — it is not a manufacturing-line quality-control tool.
...
**Fleet maintenance manager.** Picture a company that owns a bunch of delivery vans or trucks (think Amazon's delivery fleet, or a trucking company). This person's job is to keep all those vehicles running and on schedule — and they're watching continuous telemetry across the whole fleet, not waiting for a dashboard light to come on. When a vehicle's readings start trending the wrong way, their question is purely practical: *"This vehicle's readings are trending toward a fault — do I bring it in proactively, or is it fine to keep running?"* They're not fixing anything themselves — they're making a business/safety call about whether to schedule a vehicle in ahead of a breakdown or leave it running.
...
**The difference in one line:** the fleet manager asks a go/no-go triage question from an early forecast, *before* a fault has even fired; the dealer engineer asks a root-cause repair question *once* it's already there, fault already fired. Same underlying AI output (classify or forecast the fault, pull up relevant records, explain what's going on) — just a different question, asked at a different moment, depending on who's asking.
